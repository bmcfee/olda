...
What matters for segmentation is not the representation of the columns of $R$, but the similarity between them.
...
The latent structural repetition feature (\cref{fig:rep}, bottom-right) is defined as the matrix $L$:
\begin{equation}
...
proposed technique, and can be especially useful in interactive displays for data exploration.  
However, standard evaluation procedures are defined only for flat segmentations, and require a specific pruning of the segment hierarchy into some $k$ 
distinct segments.
...
\end{align*}
Intuitively, $A_\text{O}$ measures the deviation of successive segments $(c, c+1)$ from their mutual centroid $\mu_{c+}$, \ie,
the centroid resulting from merging $c$ and $c+1$.  This is exactly the type of merge operation encountered in the agglomerative
clustering algorithm described in \Cref{sec:clustering}.  Optimizing $W$ to maximize this deviation, while minimizing within-segment variance,
should enhance the overall segmentation accuracy.  While FDA has a similar objective, its relation to our specific clustering objective is more tenuous.
...
SMGA performs best on most metrics for Beatles-ISO (\cref{tab:results}, top),\footnote{SMGA parameters were tuned to 
...
and the proposed methods achieve higher accuracy for boundary detection at 0.5s resolution.

On both datasets, OLDA consistently improves performance over the unweighted model across all metrics, achieving the highest scores on SALAMI-free.
The results for FDA are mixed, and tend to degrade performance relative to the unweighted model.
