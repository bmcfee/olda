...
clustering algorithm to produce a hierarchical segmentation over the entire track in linear time. Because the
...
$(i, i+1)$, so the linkage graph forms a chain.  The full hierarchy is computed after $\Oh(t)$ merge operations, so the 
algorithm runs in $\Oh(dt)$ time. 
...
publicly available at \url{https://github.com/bmcfee/olda}.}
