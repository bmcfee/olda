...
def make_dataset(n=None, n_jobs=16, rootpath='beatles/', output_path='data/'):
    ...
    F_audio     = sorted([_ for _ in get_all_files(os.path.join(rootpath, 'audio_tut'), '.wav')])
    F_labels    = sorted([_ for _ in get_all_files(os.path.join(rootpath, 'seglab_tut'), '.lab')])
...
if __name__ == '__main__':
    ...
    with open('%s/beatles_tut_data.pickle' % output_path, 'w') as f:
