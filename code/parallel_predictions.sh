...
predictor() {
    ...
    ./feature_segmenter.py -l ${label} -b ${boundary} ${switch} "${file}" "../data/predictions/${dataset}/${algo}/${prediction}"
}
export -f predictor

parallel -j ${NUM_JOBS} predictor ${LABEL_MODEL} ${BOUNDARY_MODEL}  BEATLES_TUT olda_aic        '' {1} ::: ../data/features/BEATLES_TUT/*.pickle
...
parallel -j ${NUM_JOBS} predictor ${LABEL_MODEL} ${BOUNDARY_MODEL}  JAMS olda_aic        '' {1} ::: ../data/features/JAMS/*.pickle
